...
ENEMY_SPEED_INCREMENT = 2  # Speed increase per N coins
N = 10  # Coins required to increase enemy speed
...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect.x = random.randint(0, WIDTH - self.rect.w)
        self.rect.y = -self.rect.h  # Starts above the screen
        self.value = random.choice([1, 2, 5])  # Assign random weight to coin
...
while running:
    ...
    if collected:
        for coin in collected:
            collected_coins += coin.value  # Increase score based on coin value
        coin_sound.play()  # Play coin sound

        # Increase enemy speed every N coins collected
        if collected_coins % N == 0:
            ENEMY_SPEED += ENEMY_SPEED_INCREMENT

    # Display collected coins (Moved to left side for better visibility)
